...
function evaluateFiveCards(cards) {
    ...
    if (groups[0][1] === 4) {
        ...
        const kicker = values.find(value => value !== fourValue);
        return {
            rank: "four-of-a-kind",
            score: [
                8,
    ...
    const pairs = groups
        ...
        .sort((a, b) => b - a);
    if (pairs.length === 2) {
        const kicker = values.find(value => value !== pairs[0] &&
            value !== pairs[1]);
        return {
            rank: "two-pair",
            score: [
...
export function evaluateHand(cards) {
    ...
    const possibleHands = combinations(cards, 5);
    let bestHand = evaluateFiveCards(possibleHands[0]);
    for (let i = 1; i < possibleHands.length; i++) {
        const hand = evaluateFiveCards(possibleHands[i]);
...
//# sourceMappingURL=evaluater.js.map