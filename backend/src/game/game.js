...
export class Game {
    ...
    state;
    constructor(id, players = [], smallBlind = 10, bigBlind = 20) {
        this.id = id;
        this.state = {
    ...
    removePlayer(id) {
        if (this.state.phase !== "waiting")
            throw new Error("Cannot remove players during a hand");
    ...
    dealForPreview() {
        if (this.state.phase !== "starting")
            throw new Error("Game has not been scheduled to start");
        ...
        for (let card = 0; card < 2; card++) {
            for (const player of this.state.players)
                player.hand.push(this.state.deck.draw());
        }
        this.postBlinds();
    ...
    beginBetting() {
        if (this.state.phase !== "preview")
            throw new Error("Cards have not been dealt yet");
    ...
    postBlinds() {
        ...
        if (!smallBlind || !bigBlind)
            throw new Error("Unable to find blind players");
