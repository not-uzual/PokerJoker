export function assertCanStart(players, phase) {
    if (phase !== "waiting" && phase !== "finished")
        throw new Error("A hand has already started");
    if (players.length < 2)
        throw new Error("At least 2 players are required");
}
export function assertCanAddPlayer(phase, players, playerId) {
    if (phase !== "waiting")
        throw new Error("Cannot join a game that has already started");
    if (players.some(player => player.id === playerId))
        throw new Error("Player already exists");
}
export function assertCanAct(phase, player, currentPlayerId, playerId) {
    if (phase === "waiting" || phase === "starting" || phase === "preview" || phase === "showdown" || phase === "finished")
        throw new Error("No actions are allowed in this phase");
    if (player.folded || player.allIn)
        throw new Error("Player cannot act");
    if (currentPlayerId !== playerId)
        throw new Error("Not this player's turn");
}
export function assertActionAllowed(player, action, currentBet, minRaise, bigBlind) {
    switch (action.type) {
        case "check":
        ...
        case "bet":
            ...
            if (action.amount < bigBlind)
                throw new Error(`Minimum bet is ${bigBlind}`);
            if (action.amount > player.chips)
                throw new Error("Bet exceeds player's chips");
            return;
        case "raise": {
            ...
            if (action.amount - currentBet < minRaise && !isAllIn)
                throw new Error(`Minimum raise is ${minRaise}`);
            if (action.amount - player.currentBet > player.chips)
                throw new Error("Raise exceeds player's chips");
            return;
        }
        case "all-in":
            if (player.chips <= 0)
...
export function isBettingRoundComplete(players, currentBet) {
...
export function getSmallBlindIndex(dealerIndex, playerCount) {
    return playerCount === 2 ? dealerIndex : (dealerIndex + 1) % playerCount;
}
export function getBigBlindIndex(dealerIndex, playerCount) {
