...
function registerRoomEvents(io, socket, games, rooms) {
    socket.on("create-room", ({ name, playerId, playerName }) => {
        const roomName = name?.trim();
        const displayName = playerName?.trim();
        if (!roomName || !displayName || !playerId)
            return socket.emit("room-error", { message: "Room name and player name are required" });
        const roomId = Math.random().toString(36).substring(2, 8).toUpperCase();
        const room = { id: roomId, name: roomName, host: playerId, players: [{ playerId, name: displayName, socketId: socket.id }], maxPlayers: 9, status: "lobby" };
    ...
    socket.on("join-room", ({ roomId, playerId, playerName }) => {
        ...
        if (room.status !== "lobby")
            return socket.emit("room-error", { message: "This hand has already started. Join a new lobby for the next game." });
...
function registerGameEvents(io, socket, games, rooms, timers) {
    ...
    socket.on("game-action", ({ roomId, playerId, action }) => {
        const room = rooms.get(roomId);
...
function clearGameTimer(roomId, timers) {
    const timer = timers.get(roomId);
    if (timer)
        clearTimeout(timer);
    timers.delete(roomId);
}
function startCountdown(io, roomId, games, rooms, timers) {
...
function scheduleTurn(io, roomId, games, rooms, timers) {
    const game = games.getGame(roomId);
    ...
    timers.set(roomId, setTimeout(() => {
        ...
        if (!current || !["preflop", "flop", "turn", "river"].includes(latest.phase))
            return;
...
function emitGameState(io, roomId, games, rooms) {
    const room = rooms.get(roomId);
    if (!room)
        return;
    const game = games.getGame(roomId);
    for (const player of room.players) {
        if (player.socketId)
            io.to(player.socketId).emit("game-state", game.getPublicState(player.playerId));
    }
}
